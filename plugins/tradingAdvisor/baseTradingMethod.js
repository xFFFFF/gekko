...
var Base = function(settings) {
  ...
  if (_.size(this.indicators)) this.hasSyncIndicators = true;
};
...
Base.prototype.tick = function(candle) {
  ...
  _.each(
    this.indicators,
    function(i) {
      if (i.input === 'price') i.update(price);
      if (i.input === 'candle') i.update(candle);
    },
    this
  );
  ...
  if (!this.asyncTick) {
    ...
    var talibResultHander = function(err, result) {
      if (err) util.die('TALIB ERROR:', err);
    ...
    // handle result from talib
    _.each(this.talibIndicators, indicator =>
      indicator.run(basectx.candleProps, talibResultHander.bind(indicator))
    );

    // handle result from tulip
    var tulindResultHander = function(err, result) {
      if (err) util.die('TULIP ERROR:', err);
    ...
    _.each(this.tulipIndicators, indicator =>
      indicator.run(basectx.candleProps, tulindResultHander.bind(indicator))
    );
  }
...
Base.prototype.propogateTick = function(candle) {
  ...
  if (mode === 'realtime') {
    ...
    startTimeMinusCandleSize.subtract(
      this.tradingAdvisor.candleSize,
      'minutes'
    );
  ...
  if (done && this.finishCb) this.finishCb();
};
...
Base.prototype.addTalibIndicator = function(name, type, parameters) {
  if (!talib) util.die('Talib is not enabled');
...
Base.prototype.addTulipIndicator = function(name, type, parameters) {
  if (!tulind) util.die('Tulip indicators is not enabled');
...
Base.prototype.advice = function(newPosition, _candle) {
  ...
  if (newPosition === this._prevAdvice) return;

  // cache the candle this advice is based on
  if (_candle) var candle = _candle;
  else var candle = this.candle;
